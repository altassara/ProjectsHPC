...
PetscErrorCode ComputeRHS(KSP ksp, Vec b, void *ctx) {
  ...
  // I believe the next line should link to (https://petsc.org/release/manualpages/DMDA/DMDALocalInfo/) 
  // and/or (https://petsc.org/release/manualpages/DMDA/DMDAGetLocalInfo/#dmdagetlocalinfo)
...
PetscErrorCode ComputeMatrix(KSP ksp, Mat A, Mat P, void *ctx) {
  ...
  for (j = info.ys; j < info.ys + info.ym; j++) {
    for (i = info.xs; i < info.xs + info.xm; i++) {
      ...
      row.i = i; row.j = j;
      PetscScalar v_bottom, v_left, v_center, v_right, v_top;
      
      if (i == 0 || i == info.mx - 1 || j == 0 || j == info.my - 1) {
        
        v_bottom = 0.0; col[0].i = i; col[0].j = j;
        v_left   = 0.0; col[1].i = i; col[1].j = j;
        v_center = 1.0; col[2].i = i; col[2].j = j;
        v_right  = 0.0; col[3].i = i; col[3].j = j;
        v_top    = 0.0; col[4].i = i; col[4].j = j;
      } 
      else {
        
        v_bottom = -1.0 / hy2;               col[0].i = i;     col[0].j = j - 1;
        v_left   = -1.0 / hx2;               col[1].i = i - 1; col[1].j = j;
        v_center = (2.0/hx2) + (2.0/hy2);    col[2].i = i;     col[2].j = j;
        v_right  = -1.0 / hx2;               col[3].i = i + 1; col[3].j = j;
        v_top    = -1.0 / hy2;               col[4].i = i;     col[4].j = j + 1;
      }
