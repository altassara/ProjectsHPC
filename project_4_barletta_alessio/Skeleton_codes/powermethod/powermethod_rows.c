...
int main(int argc, char* argv[]) {
  ...
  printf("[Proc %3d] Doing rows %d to %d\n", rank, row_beg_local,
         row_end_local);
  // Partition the "n" rows of the matrix evenly among the "size" MPI
  //        processes.
  ...
  for (iter = 0; iter < niter; ++iter) {
    ...
    MPI_Allgatherv(y_local, nrows_local, MPI_DOUBLE, y, recvcounts, displ, MPI_DOUBLE, MPI_COMM_WORLD);
